...
def booking_new_trip():
    try:
        ...
        query = 'INSERT INTO data_booking (userId, destinationId, tourGuideId, bookingCode, tripDate, totalPayment, ordererNote, statusPayment, bookingDate) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, CURRENT_TIMESTAMP)'
        query_database(query, [user_id, destination_id, tour_guide_id, booking_code, trip_date, total_payment, note, 1])
        query = 'UPDATE data_pengguna SET name=%s, whatsapp=%s WHERE id=%s'
        query_database(query, [name, telephone, user_id])
        query = 'UPDATE data_pemandu_wisata SET totalTrip = totalTrip + 1 WHERE userId = %s;'
        query_database(query, [tour_guide_id])
...
def scanning_object():
    ...
    try:
        ...
        query_predicted = 'SELECT caption, photo FROM data_monumen WHERE name = %s'
        results_predicted = query_database(query_predicted, [predicted_label])
        predicted_caption = results_predicted[0]['caption']
        predicted_photo = results_predicted[0]['photo']
...
def predict_route():
    try:
        ...
        destinationss = query_database(query)
        df_destinationss = pd.DataFrame(destinationss)

        query = 'SELECT * FROM ratings'
        ratingss = query_database(query)
        df_ratingss = pd.DataFrame(ratingss)

        place_ids = df_ratingss['id'].unique().tolist()
        ...
        def get_random_recommendations():
            return np.random.choice(df_destinationss['id'], size=10, replace=False)

        if user_id in df_ratingss['User_id'].values:
            place_rated = df_ratingss[df_ratingss.User_id == user_id]
            place_not_rated = df_destinationss[~df_destinationss['id'].isin(place_rated['id'].values)]['id'].tolist()
            ...
            recommended_places = df_destinationss[df_destinationss['id'].isin(recommended_place_ids)]
            return jsonify({
                'statusCode': 200,
                'message': 'Success',
                'data': {
            ...
            random_places = df_destinationss[df_destinationss['id'].isin(random_recommendations)]
            return jsonify({
                'statusCode': 200,
                'message': 'Success',
                'data': {
                    'user_id': user_id,
                    'recommendations': random_places.to_dict('records')
                }
            }), 200
    except Exception as e:
        return jsonify({
...
def status_payment():
    try:
        ...
        query = 'SELECT * FROM data_booking WHERE bookingCode = %s'
        results = query_database(query, [bookingCode])
        print(results)
        if results == ():
            return jsonify({
                'statusCode': 400,
...
def get_user_data(userId):
    try:
        query = 'SELECT id, name, username, email FROM data_pengguna WHERE id = %s'
        results = query_database(query, (userId,))
